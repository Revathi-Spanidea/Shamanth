...
void AccelTask(void *argument)
{
    ...
    printf("Accel Task started - Reading at 10 Hz (every 100ms)\r\n");
...
void TempTask(void *argument)
{
    ...
    printf("Temp Task started - Reading at 1 Hz (every 1000ms)\r\n");
...
void StorageTask(void *argument)
{
    ...
    printf("Storage Task started - Saving data to memory\r\n");

    for(;;)
    {
        // Check accelerometer queue
        if(osMessageQueueGet(accelQueue, &accelData, NULL, 0) == osOK) {
            if(accelCount < BUFFER_SIZE) {
                accelStorage[accelCount] = accelData;
                accelCount++;
                printf("  [STORAGE] Accel sample #%d saved to RAM\r\n", accelCount);
            }
        }

        // Check temperature queue
        if(osMessageQueueGet(tempQueue, &tempData, NULL, 0) == osOK) {
            if(tempCount < BUFFER_SIZE) {
                tempStorage[tempCount] = tempData;
                tempCount++;
                printf("  [STORAGE] Temp sample #%d saved to RAM\r\n", tempCount);
            }
        }

        osDelay(10);  // Small delay to prevent starvation
    }
}
