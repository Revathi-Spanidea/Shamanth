...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Normal Condition Demo (No Priority Inversion)
  ******************************************************************************
  */
...
#define HIGH_PRIORITY_TASK_ID    2
...
volatile uint32_t low_priority_runs = 0;
...
void LowPriorityTask(void *argument)
{
    ...
    {
        low_priority_runs++;
        SafePrintf("\r\n[LOW] Attempting to acquire resource (Run #%lu)...\r\n", low_priority_runs);
        ...
        if(osMutexAcquire(resourceMutex, 1000) == osOK) {
            ...
            SafePrintf("[LOW] (High priority task will be blocked if it tries to acquire)\r\n\n");
    ...
    for(;;)
    {
        ...
        if(osMutexAcquire(resourceMutex, 10000) == osOK) {
            ...
            if(wait_time > 100) {
                ...
                SafePrintf("[HIGH] Was BLOCKED for %lu ms\r\n", wait_time);
            } else {
                SafePrintf("[HIGH] *** RESOURCE ACQUIRED *** (Attempt #%lu)\r\n", blocked_count);
                SafePrintf("[HIGH] Acquired immediately (0 ms wait)\r\n");
            }
...
int main(void)
{
    ...
    printf("3. NO medium task to interfere\r\n");
    printf("4. Low task continues running (nothing preempts it)\r\n");
    printf("5. Low task releases resource -> High task runs\r\n");
    printf("\r\n");
    printf("Task Priorities:\r\n");
    printf("  [HIGH] - Priority 2 (needs resource)\r\n");
    ...
    // Create only TWO tasks (LOW and HIGH)
