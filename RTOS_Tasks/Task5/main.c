...
//  task3Handle = osThreadNew(Task3, NULL, &task3_attr);
...
void SensorTask(void *argument)
{
    ...
    for(;;)
    {
        LIS2DU12_GetAccel(&data);
        osMessageQueuePut(accelQueue, &data, 0, osWaitForever);
        printf("SensorTask: Data Sent\r\n");
        osDelay(500);
    }
}
...
void ProcessTask(void *argument)
{
    ...
    for(;;)
    {
        ...
        {
            printf("ProcessTask: Queue get failed with status %d\r\n", status);
        }
    }
}
